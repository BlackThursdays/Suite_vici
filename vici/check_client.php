...
$log_file = 'log.txt';
// $ name_secgroup = 'Contact Center';
$ current_date  =  gmdate ( ' Ymd H: i: s ' ); // current date
// Display error
...
/**
* Check customer by phone number. Verification procedure:
 * 1. Contact persons
 * 2. Customers
...
function check_by_phone($phone)
{
    ...
    // We form the phone in two formats:
    // 375...
    // 80....
    ...
    //}
    //print_error($phone_f1);
    //print_error($phone_f2);
    // Checking contact persons
    $select_contacts = "
SELECT id
FROM contacts
WHERE regex_replace('[^0-9]', '', phone_work) = '$phone_f1' OR
    ...
    //print_error($select_contacts);
    $query = mysql_query($select_contacts);
    $clients = array();
    // If there are clients, we drive them into the $ clients array
    if (mysql_num_rows($query) > 0) {
        ...
        //print_r($clients);
        return $clients;
    } // If there are no contact persons - check for customers
    else {
        $select_accounts = "
SELECT id
FROM accounts
...
/**
 * Create a call attached to the client (lida)
 * @param $clients
 * @return string
 */
...
/**
 * Update the duration of the call at the end
 *
 * @return bool
 */
...
{
    global $current_date;
    $update_calls = "UPDATE calls JOIN calls_cstm ON calls_cstm.id_c = calls.id SET date_end = '$current_date', date_modified = '$current_date' ";
...
//    }
    if ((isset($_REQUEST['duration']) && !empty($_REQUEST['duration']))) {
        $duration_h = (int)gmdate("G", $_REQUEST['duration']);
        $duration_i = (int)gmdate("i", $_REQUEST['duration']);
        $update_calls .= " , duration_hours = $duration_h, duration_minutes = $duration_i ";
...
mysql_connect($db_server, $db_user, $db_password) or die (print_error("Ошибка подключения к БД"));
mysql_select_db($db_name) or die (print_error("Ошибка выбора БД"));
mysql_query("SET NAMES 'utf8'") or die (print_error('Ошибка утановки utf8'));
if ($_REQUEST['method'] === 'create_call') {
    ...
    // If client 1 - create a call for it and return id
    if ($clients && count($clients) == 1) {
        $call_id = create_call($clients[0], $_REQUEST['phone']);
        $return = array('type' => 'found_one', 'call_id' => $call_id);
    }
    // If there are more clients than 1, display the list of clients. By clicking on the url create a call to the client
    //elseif (count($clients) > 1) {
    //    $url = '';
    //}
    // If there are no clients, create a lead and a call for it
    else if (!$clients) {
        $call_id = create_call($clients, $_REQUEST['phone']);
        $return = array('type' => 'not_found', 'call_id' => $call_id);
    }
